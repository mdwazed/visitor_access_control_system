...
if(isset($_POST["btn_approve"])){
	...
	$status = $_REQUEST['status'];
	 ...
	 while (list ($key,$val) = @each ($status)) {
		...
		$query_app = "Update tbl_pass_temporary set `approvalStatus` = 1 where `visitorName`='".$val."' ";      ///  could it chk with  other ID??  ??
...
if(isset($_POST["btn_deny"])){
	...
	$status = $_REQUEST['status'];
	 ...
	 while (list ($key,$val) = @each ($status)) {
		...
		$query_app = "Update tbl_pass_temporary set `approvalStatus` = 2 where `visitorName`='".$val."' ";      ///  could it chk with  other ID??  ??
		$result_app =	$db->execQuery($query_app) or die("Error: " . mysql_error());
	} 
}
		...
		<TD>	
        	...
        	<table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#BFBFBF" class="bodytext" style="									                                                                                        border-collapse: collapse">
                          ...
                          <tr>
                                ...
                                <td width="22%"  bgcolor="#DFDFDF" align="center"><strong>Visitor Name</strong></td>
                                ...
                                <td width="8%"  bgcolor="#DFDFDF" align="center"><strong>Directorate </strong></td>
...
<?php
		...
		$sql_search = "SELECT rowID,visitorName,visitorType FROM tbl_pass_temporary  WHERE approvalStatus = 0 ORDER BY rowID DESC LIMIT ".$offset.",".$rowsPerPage;
...
<?php		
				...
				for($i=0; $i<$num_of_rows_found; $i++){
					...
					$sql = "SELECT visitorAddress, directorateToVisit,purposeOfVisit,startDate,endDate FROM tbl_pass_temporary WHERE visitorName = "."'".$result_array[$i]['visitorName']."'";
					$result_all_rows = $db->execQuery($sql);
					$result_array_registration = $db->resultArray($result_all_rows);	  		
?>						 

<form name="temporary_approval" action="<?php echo $_SERVER['PHP_SELF']; ?>?action=temporary_approval" method="post" onsubmit="return validate();">
		...
		<tr>
            ...
            <td bgcolor="#EEEEEC" align="center"><?php  if (isset($result_array_registration[0]['startDate'])) echo $result_array_registration[0]['startDate'];?></td>
            <td bgcolor="#EEEEEC" align="center"><?php  if (isset($result_array_registration[0]['endDate'])) echo $result_array_registration[0]['endDate'];?></td>
...
<?php
		...
		$query = "SELECT * FROM tbl_pass_temporary where approvalStatus = 0";		
		...
		for($page = 1; $page <= $maxPage; $page++)
		{
		   ...
		   {
			  $nav .= " <a href=\"$self?action=temporary_approval&page=$page\">$page</a> ";
		   } 
		}
		...
		{
		   $page  = $pageNum - 1;
		   $prev  = " <a href=\"$self?action=temporary_approval&page=$page\">[Prev]</a> ";
		
		   $first = " <a href=\"$self?action=temporary_approval&page=1\">[First Page]</a> ";
		} 
		else
		{
		...
		{
		   $page = $pageNum + 1;
		   $next = " <a href=\"$self?action=temporary_approval&page=$page\">[Next]</a> ";
		
		   $last = " <a href=\"$self?action=temporary_approval&page=$maxPage\">[Last Page]</a> ";
		} 
		else
		{
