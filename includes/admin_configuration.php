...
if(isset($_POST['submit_archive_delete'])){
		...
		$from_date = $db->changeFormatDate($_POST['from_date']);
		$to_date = $db->changeFormatDate($_POST['to_date']);
		
		// @@@@@@@@@ added below to shift data to tbl_deleted_pass     @@@@@
		
		$sql_insert = sprintf("INSERT INTO tbl_deleted_pass (passID,visitorName,visitorType,visitorAddress,directorateToVisit,purposeOfVisit,scheduledVisitDate,isMultipleEntry,passCreator,sentryUserName,mobileNumber,nationalID, 	visitorCardNo,visitorInTime,visitorOutTime,	passStatus, adminNotified)
		SELECT passID,visitorName,visitorType,visitorAddress,directorateToVisit,purposeOfVisit,scheduledVisitDate,isMultipleEntry,passCreator,sentryUserName,mobileNumber,nationalID, 	visitorCardNo,visitorInTime,visitorOutTime,	passStatus, adminNotified FROM tbl_archive_pass WHERE scheduledVisitDate BETWEEN '%s' AND '%s'",		
		$from_date, $to_date )or die(mysql_error());	
...
<table width="100%" >
    ...
    <form  action="<?php echo $_SERVER['PHP_SELF']; ?>?action=admin_configuration" method="post" onsubmit="return validateForm();" >
        ...
        <tr>
                  ...
                  <td colspan="2"><input type="text" name="from_date" id="datepicker_past3" /></td>
        </tr>
		 ...
		 <tr>
                  ...
                  <td width="32" align="center">:</td>
                  <td colspan="2"><input type="text" name="to_date" id="datepicker_past4" /></td>
        </tr>
