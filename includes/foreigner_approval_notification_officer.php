...
<a href="<?php echo $_SERVER['PHP_SELF']; ?>?action=foreigner_approval_notification_officer&page=<?php if(isset($_REQUEST['page'])) echo $_REQUEST['page']; else echo 1; 		 							?>&notified=true&passID=<?php echo $result_array[$i]['rowID'];?>">Notified</a>
            </td>
...
//		$query = "SELECT * FROM tbl_pass_temporary " . $whereQ ;
 $query = "SELECT * FROM tbl_pass_foreigner ".$whereQ ;				
		...
		for($page = 1; $page <= $maxPage; $page++)
		{
		   ...
		   {
			  $nav .= " <a href=\"$self?action=foreigner_approval_notification_officer&page=$page\">$page</a> ";
		   } 
		}
		...
		{
		   $page  = $pageNum - 1;
		   $prev  = " <a href=\"$self?action=foreigner_approval_notification_officer&page=$page\">[Prev]</a> ";
		
		   $first = " <a href=\"$self?action=foreigner_approval_notification_officer&page=1\">[First Page]</a> ";
		} 
		else
		{
		...
		{
		   $page = $pageNum + 1;
		   $next = " <a href=\"$self?action=foreigner_approval_notification_officer&page=$page\">[Next]</a> ";
		
		   $last = " <a href=\"$self?action=foreigner_approval_notification_officer&page=$maxPage\">[Last Page]</a> ";
		} 
		else
		{
