...
if(isset($_POST['submit'])){
    ...
    if(   (empty($_POST['mobile_number']) && empty($_POST['national_ID']))  ||  empty($_POST['card_number']) ){
		...
		$query = sprintf("UPDATE tbl_archive_pass SET mobileNumber = '%s', nationalID = '%s', visitorCardNo = '%s', visitorInTime = '%s', passStatus = '%s'
...
$sql_search = "SELECT * FROM tbl_archive_pass WHERE rowID = '" . $_REQUEST['passID'] . "'" ;
...
<table width="100%" >
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>?action=pass_details&passID=<?php echo $result_array[0]['rowID'];?>" method="post">
     
        <table  bgcolor="#EEEEEC" width="100%" align="right" >

<?php 
        if ($result_array[0]['isMultipleEntry']=="yes"){
?>        
        <tr bgcolor="#FF0033">
                    <td colspan="6" align="center">Multiple Entry Allowed</td>   
        </tr>
<?php 
}
?>
...
if($result_array[0]['passStatus']!='open'){
	...
	$image_Visitor_Photo = ("VisitorPhotos/".$visitorType."_".$visitorPassID. ".jpg");
?>            
        <tr>
          ...
          <td valign="top" align="left">
                 <?php 
				   if( ($result_array[0]['passStatus'] == 'open')&& ($_SESSION['_userType'] !=1 )){ 
					  ...
					  $_SESSION['visitorType'] = 'GV';  // General Visitor
...
<!--          			<input type="button" name="photoBtn" id="photoBtn" value="Take Photo" onclick="parent.location='CapturePhoto/index.html'" -->
          			<input type="button" name="photoBtn" id="photoBtn" value="Take Photo" onclick="parent.window.open('CapturePhoto/index.php','thePopup', width='800', height='500', top='10', left='10');" />                    
		...
		<tr>
                    <td width="232" align="right"><strong>Directorate to Visit</strong></td>
                  <td align="center">:</td>                                    
                  <td><?php 	
...
<tr>
                  <td width="232" align="right"><strong>Purpose of Visit</strong></td>
                  <td align="center">:</td>                                    
        ...
        <tr>
                  <td width="232" align="right"><strong>Visit Date</strong></td>
                  <td align="center">:</td>
                  <td><?php echo $result_array[0]['scheduledVisitDate'];?></td>
